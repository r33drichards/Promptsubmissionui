...
/**
 * Hook to stop a running session.
 *
 * Stopping a session kills the running process but preserves the sandbox,
 * allowing the session to be resumed later with a new prompt.
 *
 * @example
 * ```tsx
 ...
 *
 * const handleStop = () => {
 *   stopSession.mutate('session-123');
 * };
 * ```
 */
export function useStopSession(
  ...
  return useMutation({
    ...
    onSuccess: (data, id, context) => {
      // Invalidate the session detail to fetch updated status
      queryClient.invalidateQueries({
        queryKey: queryKeys.sessions.detail(id),
      });
